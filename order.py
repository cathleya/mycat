...
#
import MySQLdb,string,sys,random
...
try:
	...
	# random users
	for ru in range(1,100):
		GenUser="""
			insert into customer_login(login_name) 
			values('%s')
		"""%(faker.name())
		cursor.execute(GenUser)
	...
	for i in range(0,10):
		...
		InSQL="""
             insert into order_master(order_sn,customer_id,province,city,district,address
               ,order_money,create_time)
             values('%s',%d,%d,%d,%d,'%s',3,now())
		"""%(order_sn,customer_id,province,city,district,faker.address())
		#print InSQL
		...
		InsSQL="""
          insert into order_detail(order_id,product_name,product_price)
          values(%d,'%s',%d)
        """%(order_id,faker.name(),random.randint(10,1000))
		#print InsSQL
